...
`/` (главная, `public/index.html` + `app.js`) сама ничего не знает про телефон/OTP: если пользователь не авторизован или профиль не создан — редирект на `/login`. `/login` (`public/login.html` + `login.js`), в свою очередь, если пользователь уже авторизован и профиль есть — сразу редиректит на `/`. Оба редиректа — обычные `window.location.href`, роутера/SPA-навигации нет.
...
- `public/login.html` + `public/login.js` — страница `/login`: телефон → SMS-код → создание профиля
...
- `public/auth.js` — телефонная авторизация, создание профиля, выход
